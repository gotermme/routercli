...
// cmd/cmd_*.go file ever having registered it, a mistake that would
...
// written cmd/cmd_*.go file, see this package's own doc comment on
...
// registered, see registry.go, meaning some cmd/cmd_*.go file's
...
func VerifyCommandLevels(levels *TreeStructure) []error {
	...
	for _, level := range levels.Order {
		...
		if _, ok := lookupHandler(level.EnterCommand); !ok {
			problems = append(problems, fmt.Errorf("command level %q declares enter_command %q, but no cmd/cmd_*.go file has registered a command by that name", level.Name, level.EnterCommand))
		}
		if level.ExitCommand != "" {
			if _, ok := lookupHandler(level.ExitCommand); !ok {
				problems = append(problems, fmt.Errorf("command level %q declares exit_command %q, but no cmd/cmd_*.go file has registered a command by that name", level.Name, level.ExitCommand))
			}
		}
	}
