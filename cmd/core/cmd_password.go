...
// runPasswordChange - This function is the registered "password
// change" handler. It carries the fixed command.HandlerFunc signature
// command.Register requires, so it simply passes the real process's
// stdin file descriptor and stdout along to runPasswordChangeWithIO,
// which does the actual work. See this file's init doc comment for
// the shape of the whole flow and why the attempt budget is shared
...
// file descriptor injection pattern main.go's establishSession
// already uses, and for the same reason: it lets a test hand this
// function a pty's slave file directly rather than needing a real
// terminal attached to the test binary itself. runPasswordChange
// above is the only real caller outside a test, always passing the
// real process's own stdin and stdout, so production behavior is
// unchanged.
